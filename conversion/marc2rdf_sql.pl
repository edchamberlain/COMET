...
# Marc21 to RDF SQl based conversion script
...
if ($ARGV[0]) {
         ...
         getopts('o:u:d:f:');
         my ($marcIDFile,$outputFilename);
        
         # optional ID filename, else takes the argument ...
         if ($opt_f) {
           $marcIDFile=$opt_f;      
         } else {
            $marcIDFile = $ARGV[0];   
         ...
         if ($opt_o) {
            ...
            print "Outputting to $outputFilename \n";
             ...
             $outputFilename = "$marcIDFile" . "_triples.nt";
         }
         ...
         my $un = "";
         my $pw = "";
         my $dbh=DBI->connect("$driver:$database2, "$un","$pw") or die "Could not connect to database: $!";
         ...
         # Open MARC ID file
         open my $idFh, '<', $marcIDFile or die "Cannot open: $!";
          
         my $count=0;
         my $query = "scripts/comet";
         my $path="/home/ucat/$query/";
         
         print "Please ensure you have entered export LANG='en_GB' at the command line before running this! \n";
         print  "PATH IS $path \n";
         
         
         
         while (my $recordId = <$idFh>) {
                  print $recordId . "\n";
                  my $record ='';
                  my $field_001='';
                  eval{
                  $record = get_bib_marc_record($dbh, $recordId, $path);
                  };
                   if ($@){

	            print LOG "FAILED TO OPEN  $recordId \n";
                    next;
                    
                   }else{
                           
                  if ($record) {
                          $field_001=$record->field('001');
                  
                           if ($field_001){ # added as some UL records return false positives 1043240!
...
sub get_bib_marc_record{
    ...
    $sth_marc->execute($bib_id);

    while (my $marc_segment=$sth_marc->fetchrow_array()){
        $marc.=$marc_segment;
    }
     
    my $marc_filename="bib_marc.mrc";
  
   # print $marc_filename;
    
    open (MARC_FILE, "> $marc_filename") or die "Could not open marc writey file: $!";
    print MARC_FILE "$marc";
        
    close (MARC_FILE);

#generates MARC object for bib record
    my $x=MARC::Record->new("$marc_filename");
